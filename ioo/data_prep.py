...
class FusionData(object):
    ...
    def __init__(self, filename:list, datatype:list = ['chinese_mosiac', 'FY4A'], \
        ...
        self.Lat_des_1D = np.arange( Max_Lat, Min_Lat, -1*self.resolution )  # 生成插值后的纬度
        self.Lon_des_1D = np.arange( Min_Lon, Max_Lon,  self.resolution )  # 生成插值后的经度
    ...
    def read_interp(self, filename:str, datatype:str, var_98d:list=['FH', 'compz']):
        ...
        Lon_des_2D, Lat_des_2D = np.meshgrid(self.Lon_des_1D, self.Lat_des_1D) 
